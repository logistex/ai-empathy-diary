...
| 409 | `CONFLICT` | 리소스 상태 충돌(예: 이미 분석된 일기의 재분석) |
...
|---|---|---|
| 미로그인 | 401 | `UNAUTHORIZED` |
| 잘못된 `id`(10진수 정수 아님) | 400 | `VALIDATION_ERROR` |
| 존재하지 않거나 본인 일기가 아님 | 404 | `NOT_FOUND` |
| **이미 분석된 일기**(재분석 불가, 다운그레이드 방지) | 409 | `CONFLICT` |
| DB 갱신 실패 등 내부 오류 | 500 | `INTERNAL_ERROR` |

> 재분석은 `emotion`이 `null`(미분석)인 일기에만 적용된다. 이미 분석된 일기에 호출하면 기존 결과를 보호하기 위해 `409`를 반환한다(프런트는 미분석 항목에만 버튼을 노출한다).
...
## 5. `DELETE /api/account` — 회원 탈퇴 (로그인 필요)

로그인한 사용자 **본인의 계정과 모든 일기를 영구 삭제**한다. 되돌릴 수 없다.

### 요청

- 바디 없음. 세션의 `user_id` 본인 계정만 대상.
...
- `diary.users` 행을 삭제하며, `diary_entries` 는 FK `ON DELETE CASCADE` 로 함께 삭제된다.
- 클라이언트는 성공 후 `signOut()` 으로 세션을 정리하고 홈으로 이동한다.
